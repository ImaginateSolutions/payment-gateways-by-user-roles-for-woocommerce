...
class Alg_WC_Payment_Gateways_by_User_Roles_Settings_General extends Alg_WC_Payment_Gateways_by_User_Roles_Settings_Section {
	...
	function get_user_roles_options() {
		global $wp_roles;
		$all_roles = array_merge( array( 'guest' => array( 'name' => __( 'Guest', 'payment-gateways-by-user-roles-for-woocommerce' ), 'capabilities' => array() ) ),
		...
		foreach ( $gateways as $key => $gateway ) {
			$gateways_settings = array_merge( $gateways_settings, array(
				array(
					...
					'desc'     => ( ! in_array( $key, array( 'bacs', 'cheque', 'cod', 'paypal' ) ) ? apply_filters( 'alg_wc_payment_gateways_by_user_roles_settings',
						'<span style="background-color: #fafafa; padding: 10px;">'. sprintf( 'You will need %s plugin to set roles for "%s" gateway.',
				...
				array(
					...
					'css'      => 'width: 100%;',
					'options'  => $user_roles,
					'custom_attributes' => array_merge( array( 'data-placeholder' => __( 'Select user roles...', 'payment-gateways-by-user-roles-for-woocommerce' ) ),
						( ! in_array( $key, array( 'bacs', 'cheque', 'cod', 'paypal' ) ) ?
